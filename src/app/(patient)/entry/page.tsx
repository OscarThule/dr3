...
const getConsultationFee = (center: MedicalCenter): number => {
  if (center?.paymentSettings?.consultationFee !== undefined) {
    return center.paymentSettings.consultationFee;
  }
  return center?.settings?.consultationCosts?.private?.faceToFace || 0;
};

const getOnlineConsultationFee = (center: MedicalCenter): number => {
  if (center?.paymentSettings?.onlineConsultationFee !== undefined) {
    return center.paymentSettings.onlineConsultationFee;
  }
...
const getDepositAmount = (center: MedicalCenter): number => {
  if (center?.paymentSettings?.bookingDeposit !== undefined) {
    return center.paymentSettings.bookingDeposit;
  }
  const consultationFee = getConsultationFee(center);
  const depositPercentage = center?.paymentSettings?.depositPercentage || 0;
  return depositPercentage > 0 ? (consultationFee * depositPercentage) / 100 : 0;
};

const getRemainingAmount = (center: MedicalCenter): number => {
  if (center?.paymentSettings?.remainingAmount !== undefined) {
    return center.paymentSettings.remainingAmount;
  }
  const consultationFee = getConsultationFee(center);
...
const arePaymentsEnabled = (center: MedicalCenter): boolean => {
  // Type-safe check for enablePayments with fallback
  const paymentSettings = center?.paymentSettings as any;
  return paymentSettings?.enablePayments || false;
};

const getPaymentMethods = (center: MedicalCenter): string[] => {
  // Type-safe check for paymentMethods with fallback
  const paymentSettings = center?.paymentSettings as any;
  return paymentSettings?.paymentMethods || ['cash'];
};

const getCurrency = (center: MedicalCenter): string => {
  // Type-safe check for currency with fallback
  const paymentSettings = center?.paymentSettings as any;
...
export default function EntryPage() {
  ...
  // Quick filter buttons - Add price filters
  const filterButtons = [
    ...
    { id: 'affordable' as FilterType, label: 'Under R500', count: medicalCenters.filter(c => getConsultationFee(c) < 500).length },
    { id: 'premium' as FilterType, label: 'Premium', count: medicalCenters.filter(c => getConsultationFee(c) > 800).length },
  ];
          ...
          <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-blue-500 to-purple-700 rounded-xl sm:rounded-2xl p-5 sm:p-6 text-white shadow-lg">
            <div className="relative z-10">
              ...
              <p className="text-blue-100 text-sm sm:text-base md:text-lg mb-6 sm:mb-8">
                Compare prices, specialties, and book appointments instantly
              </p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-grow">
                      <div className="text-xl sm:text-2xl font-bold mb-1">
                        R {medicalCenters.length > 0 
                          ? Math.min(...medicalCenters.map(c => getConsultationFee(c))).toFixed(0)
                          : '0'}
                      </div>
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <div className="mt-2 sm:mt-0">
                              <div className="inline-flex items-center bg-gradient-to-r from-green-500 to-emerald-600 text-white px-3 py-1.5 rounded-lg shadow-sm">
                                <span className="font-bold text-lg">{formatCurrency(consultationFee, center)}</span>
                        ...
                        <div className="border border-gray-200 rounded-lg overflow-hidden">
                          ...
                          <div className="p-3">
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <span className="font-bold text-gray-800">{formatCurrency(consultationFee, center)}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-sm text-gray-600">Online:</span>
                              <span className="font-bold text-gray-800">{formatCurrency(onlineFee, center)}</span>
                            </div>
                            {depositAmount > 0 && (
                              <div className="mt-3 pt-2 border-t border-gray-100">
                                <div className="flex justify-between items-center text-sm">
                                  <span className="text-gray-600">Deposit:</span>
                                  <span className="font-semibold text-blue-600">{formatCurrency(depositAmount, center)}</span>
                                </div>
                          ...
                          <div className="border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div className="p-3">
                              {governmentFaceToFace > 0 && (
                                <div className="flex justify-between items-center mb-2">
                                  <span className="text-sm text-gray-600">Face-to-Face:</span>
                                  <span className="font-bold text-green-700">{formatCurrency(governmentFaceToFace, center)}</span>
                                </div>
                              )}
                              {governmentOnline > 0 && (
                                <div className="flex justify-between items-center">
                                  <span className="text-sm text-gray-600">Online:</span>
                                  <span className="font-bold text-green-700">{formatCurrency(governmentOnline, center)}</span>
                                </div>
                              )}
                              <div className="mt-3 pt-2 border-t border-gray-100">
                        ...
                        <div className="mt-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {paymentMethods.map((method: string, index: number) => {
                              const methodConfig = {
                                'credit_card': { label: 'Credit Card', icon: '💳', color: 'bg-blue-100 text-blue-800' },
                                'debit_card': { label: 'Debit Card', icon: '💳', color: 'bg-indigo-100 text-indigo-800' },
                                'cash': { label: 'Cash', icon: '💰', color: 'bg-green-100 text-green-800' },
                                'insurance': { label: 'Insurance', icon: '🏥', color: 'bg-purple-100 text-purple-800' },
                                'eft': { label: 'EFT', icon: '🏦', color: 'bg-yellow-100 text-yellow-800' },
                              }[method] || { label: method, icon: '💳', color: 'bg-gray-100 text-gray-800' };
                              
                              return (
                                <span key={index} className={`px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5 ${methodConfig.color}`}>
